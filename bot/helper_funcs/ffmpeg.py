...
async def convert_video(video_file, output_directory, total_time, bot, message, chan_msg):
   ...
   ## "-metadata", "title=@HG_Anime", "-vf", "drawtext=fontfile=njnaruto.ttf:fontsize=20:fontcolor=black:x=15:y=15:text=" "",
    ...
    file_genertor_command = f"""ffmpeg -hide_banner -loglevel quiet -progress '''{progress}''' -i '''{video_file}''' -filter_complex "drawtext=fontfile=njnaruto(1).ttf:fontsize=29:fontcolor=white:bordercolor=black@0.50:x=w-tw-10:y=10:box=1:boxcolor=black@0.5:boxborderw=6:text='ᴛᴇʟᴇɢʀᴀᴍ~ʜɢ_ᴀɴɪᴍᴇ':enable='between(t,0,15)':alpha='if(lt(t,14)\,1\,if(lt(t\,15)\,(1-(t-14))/1\,0))', drawtext=fontfile=njnaruto(1).ttf:text='ᴛᴇʟᴇɢʀᴀᴍ~ʜɢ_ᴀɴɪᴍᴇ':bordercolor=black@0.50:borderw=5:fontcolor=white:fontsize=29:x=w-((2*w-200)*(t-615)/60):y=lh+0.5:enable='between(t, 615,680)':alpha='if(lt(t,679)\,1\,if(lt(t\,680)\,(1-(t-679))/1\,0))'[out1]" -metadata:s:a:0 title="ᴛᴇʟᴇɢʀᴀᴍ~ʜɢ_ᴀɴɪᴍᴇ" -metadata:s:a:1 title="ᴛᴇʟᴇɢʀᴀᴍ~ʜɢ_ᴀɴɪᴍᴇ" -metadata:s:s:0 title="HG Anime" -metadata:s:s:1 title="HG Anime" -map [out1] -map 0:a? -map 0:s? -map 0:t? -metadata title="HG Anime" -c:v {codec[0]} -crf {crf[0]} -c:s copy -pix_fmt yuv420p -s {resolution[0]} -b:v 150k -c:a libopus -b:a {audio_b[0]} -preset {preset[0]}  '''{out_put_file_name}''' -y"""
 #Done !!
